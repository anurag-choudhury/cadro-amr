...
def generate_launch_description():
    pkg_cadro_nav=get_package_share_directory("cadro_nav")
    pkg_nav2_bringup=get_package_share_directory("nav2_bringup")
    
    use_sim_time = LaunchConfiguration('use_sim_time', default='true')
	# with_rviz = LaunchConfiguration('with_rviz', default='true')
    map_yaml_file = LaunchConfiguration('map',
                default=os.path.join(pkg_cadro_nav, 'maps', 'map.yaml'))
    nav2_config_file = LaunchConfiguration('params', 
                    default=os.path.join(pkg_cadro_nav, 'config', 'nav2_params.yaml'))
